<template>
  <div>
    <div class="setup-container" v-if="!isStarted">
      <Loading v-if="categories.length == 0" text="Идет загрузка..." />
      <div v-else>
        <h1>Кафедра №13!</h1>
        <h2>Выберите трудность</h2>
    ...
    <div v-if="isStarted">
      <Loading v-if="questions.length == 0" text="Формирование теста..." />
      <div
        class="quiz"
      ...
      >
        ...
        <div class="restart-buttons">
          ...
          <button class="button" type="button" @click="resetQuestions()">Выбрать другую тему</button>
          <button class="button" type="button" @click="resetAll()">Выбрать другую настройку теста</button>
        </div>
      </div>
    </div>
  </div>
...
const difficulties = [
  { level: null, name: 'Выберите сложность' },
  { level: 'easy', name: 'На оценку 3' },
  { level: 'medium', name: 'На оценку 4' },
  { level: 'hard', name: 'На оценку 5' },
];
...
export default {
  ...
  methods: {
    ...
    completionMessage() {
      let text = 'В следующий раз повезет больше';
